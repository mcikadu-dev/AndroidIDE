...
open class TextViewAdapter<T : TextView> : ViewAdapter<T>() {
  ...
  override fun createAttrHandlers(create: (String, AttributeHandlerScope<T>.() -> Unit) -> Unit) {
    ...
    create("textSize") {
      view.setTextSize(
        ...
        parseDimensionF(context, value, SizeUtils.sp2px(12f).toFloat())
      )
    }
  ...
  override fun createUiWidgets(): List<UiWidget> {
    val klass = TextView::class
    return listOf(
      UiWidget(
        klass.java,
        klass.simpleName ?: klass.java.simpleName,
        R.drawable.ic_palette_text_view
      )
    )
  }
