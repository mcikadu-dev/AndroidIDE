...
class AndroidManifestBuilder {
  ...
  private fun manifestSrc(): String {
    val rawXml = IndentedXmlBuilder(autoIndent = true).apply {
      buildManifest()
    }.withXmlDecl()
  ...
  private fun formatXml(xml: String): String {
    return xml
      .replaceFirst(Regex("\\?>\\s*<manifest"), "?>\n<manifest")
      .replace(Regex("(?m)(\\s*)<(\\w+)((?:\\s+\\w+:[\\w]+=\"[^\"]+\")+)(\\s*/?>)")) { m ->
        val (indent, tag, attrs, closing) = m.destructured
        val parts = attrs.trim().split(Regex("\\s+(?=\\w+:[\\w]+=\"[^\"]+\")"))
        if (parts.size == 1) {
          "$indent<$tag ${parts[0]}$closing"
        } else {
          "$indent<$tag" + parts.joinToString("$indent    ", prefix = "$indent    ", postfix = closing)
        }
      }
  }
