...
inline fun baseProject(projectName: StringParameter = projectNameParameter(),
  ...
  return ProjectTemplateBuilder().apply {
    ...
    projectName.observe { name ->
      val newPackage =
        AndroidUtils.projectNameToPackageName(name.value, packageName.value)
    ...
    preRecipe = {
      ...
      val newProjectName = projectName.value.replace(Regex("\\s+"), "")

      this@apply._data = ProjectTemplateData(
        projectName.value, File(saveLocation.value, newProjectName), projectVersionData,
