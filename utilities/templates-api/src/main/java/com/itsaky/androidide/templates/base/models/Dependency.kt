...
data class Dependency(val configuration: DependencyConfiguration,
  ...
  fun platformValue(useKts: Boolean): String {
    return if (useKts)
      """${configuration.configName}(platform("${group}:${artifact}${optionalVersion()}"))""".trim()
    else
      """${configuration.configName} platform('${group}:${artifact}${optionalVersion()}')""".trim()
  }

  fun value(useKts: Boolean): String {
    return if (useKts)
      """${configuration.configName}("${group}:${artifact}${optionalVersion()}")""".trim()
    else
      """${configuration.configName} '${group}:${artifact}${optionalVersion()}'""".trim()
  }
