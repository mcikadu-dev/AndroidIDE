...
class AndroidModuleTemplateBuilder : ModuleTemplateBuilder() {
  ...
  /**
   * Set whether this Android module uses ViewBinding or not.
   *
   * If this is set to `true`, then ViewBinding will be enabled via
   * `buildFeatures { viewBinding = true }` in the `build.gradle[.kts]` file.
   *
   * If this is set to `false`, no ViewBinding-related configuration will be added.
   * Note that when [isComposeModule] is `true`, Compose configuration takes
   * precedence and ViewBinding will be ignored.
   */
  var viewBindingEnabled = true
  ...
  override fun RecipeExecutor.buildGradle() {
    save(buildGradleSrc(isComposeModule, viewBindingEnabled), buildGradleFile())
  }
