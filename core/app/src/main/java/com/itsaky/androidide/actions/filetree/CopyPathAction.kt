...
class CopyPathAction(context: Context, override val order: Int) :
  BaseFileTreeAction(
    context,
    labelRes = R.string.copy_path,
    iconRes = R.drawable.ic_copy_path
  ) {
  ...
  override suspend fun execAction(data: ActionData) {
    ...
    val projectPath = "${projectDir.name}${File.separator}$relativePath"

    ClipboardUtils.copyText("[AndroidIDE] Copied File Path", projectPath)
