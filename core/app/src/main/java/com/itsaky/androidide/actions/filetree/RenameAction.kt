...
class RenameAction(context: Context, override val order: Int) :
    ...
    builder.setPositiveButton(com.itsaky.androidide.resources.R.string.rename_file) {
      ...
      actionScope.launchAsyncWithProgress(
          ...
          action = { _, _ ->
            ...
            val renamed = if (isGradleModule(file)) {
              renameGradleModule(file, name)
            } else {
              name.length in 1..40 && FileUtils.rename(file, name)
            }
  ...
  private fun renameGradleModule(oldDir: File, newName: String): Boolean {
    if (newName.isBlank()) return false

    val projectDir = File(IProjectManager.getInstance().projectDirPath)

    val oldRelative = oldDir.relativeTo(projectDir)
    ...
    val newRelative = newDir.relativeTo(projectDir)
      .path.replace(File.separatorChar, ':')

    // Update settings.gradle
    findSettingsGradle(projectDir)?.let { (file, _) ->
      val content = file.readText()
      val newContent = content.replace(":$oldRelative", ":$newRelative")
